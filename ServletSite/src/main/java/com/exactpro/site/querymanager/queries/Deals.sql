...
FROM deals
WHERE
    ((STR_TO_DATE(:deal_date_before, '%Y-%m-%d') IS NULL OR STR_TO_DATE(:deal_date_after, '%Y-%m-%d') IS NULL)
        OR (deal_date >= UNIX_TIMESTAMP(STR_TO_DATE(:deal_date_before, '%Y-%m-%d')) AND deal_date <= UNIX_TIMESTAMP(STR_TO_DATE(:deal_date_after, '%Y-%m-%d')) )) AND
